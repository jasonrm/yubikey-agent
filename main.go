...
func (a *Agent) List() ([]*agent.Key, error) {
	...
	for _, slot := range slots {
		...
		var cn string
		var err error
		pk, cn, err = getPublicKey(a.yk, slot)
		if err != nil {
			continue
		}
		k := &agent.Key{
...
func getPublicKey(yk *piv.YubiKey, slot piv.Slot) (ssh.PublicKey, string, error) {
	...
	return pk, cert.Subject.CommonName, nil
}
...
func (a *Agent) signers() ([]ssh.Signer, error) {
	...
	for _, slot := range slots {
		pk, _, err := getPublicKey(a.yk, slot)
		if err != nil {
			continue
		}
